...
def render_analytics():
    ...
    def fetch_data():
        ...
        try:
            state["data"] = get_all_analytics_data(
                db_sess, hunt_id=h_id, days=state["time_range_days"]
            )
        finally:
            db_sess.close()
    ...
    with ui.column().classes("w-full gap-0"):
        ...
        with ui.row().classes("w-full justify-between items-center flex-wrap gap-4 mb-[22px]"):
            with ui.column().classes("gap-0"):
                ...
                ui.label(
                    "Real-time recruitment funnel, velocity, outreach response and AI cost metrics."
                ).classes("th-muted")
            ...
            with ui.row().classes("items-center gap-3 flex-wrap"):
                ...
                ui.button("CSV", icon="table_view", on_click=lambda: handle_export("csv")).classes(
                    "th-slate-btn"
                ).tooltip("Download CSV report")
                ui.button("Excel", icon="grid_on", on_click=lambda: handle_export("xlsx")).classes(
                    "th-slate-btn"
                ).tooltip("Download Excel workbook")
                ui.button(
                    "PDF", icon="picture_as_pdf", on_click=lambda: handle_export("pdf")
                ).classes("th-primary-btn").tooltip("Download PDF report")
            ...
            with kpi_container:
                ...
                with ui.card().classes("col p-4 th-card border border-teal-500/20"):
                    with ui.row().classes("justify-between items-center w-full mb-1"):
                        ui.label("Active Hunts").classes(
                            "text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        )
                        ui.icon("search", size="sm", color="teal-4")
                    ui.label(str(kpi.get("active_hunts", 0))).classes(
                        "text-3xl font-bold text-teal-400"
                    )
                    ui.label(f"{kpi.get('total_hunts', 0)} total campaigns").classes(
                ...
                with ui.card().classes("col p-4 th-card border border-teal-500/20"):
                    with ui.row().classes("justify-between items-center w-full mb-1"):
                        ui.label("Sourced Candidates").classes(
                            "text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        )
                        ui.icon("groups", size="sm", color="amber-4")
                    ui.label(str(kpi.get("total_sourced", 0))).classes(
                        "text-3xl font-bold text-amber-400"
                    )
                    ui.label(
                        f"{kpi.get('interviewing_candidates', 0)} currently interviewing"
                    ).classes("text-xs text-slate-500 mt-1")

                # Conversion Rate
                with ui.card().classes("col p-4 th-card border border-teal-500/20"):
                    with ui.row().classes("justify-between items-center w-full mb-1"):
                        ui.label("Funnel Conversion").classes(
                            "text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        )
                        ui.icon("trending_up", size="sm", color="emerald-4")
                    ui.label(f"{kpi.get('conversion_rate', 0)}%").classes(
                        "text-3xl font-bold text-emerald-400"
                    )
                    ui.label(f"{kpi.get('hired_candidates', 0)} candidate hires").classes(
                        "text-xs text-slate-500 mt-1"
                    )
                ...
                with ui.card().classes("col p-4 th-card border border-teal-500/20"):
                    with ui.row().classes("justify-between items-center w-full mb-1"):
                        ui.label("Time-to-Fill").classes(
                            "text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        )
                        ui.icon("schedule", size="sm", color="indigo-4")
                    time_to_fill = kpi.get("avg_time_to_fill_days", 0)
                    ui.label(f"{time_to_fill}d" if time_to_fill else "—").classes(
                ...
                with ui.card().classes("col p-4 th-card border border-teal-500/20"):
                    with ui.row().classes("justify-between items-center w-full mb-1"):
                        ui.label("Recorded AI Savings").classes(
                            "text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        )
                        ui.icon("savings", size="sm", color="cyan-4")
                    ui.label(f"${kpi.get('estimated_cost_saved', 0.0)}").classes(
            ...
            with charts_container:
                ...
                with ui.row().classes("w-full gap-6 no-wrap-md"):
                    ...
                    with ui.card().classes("col-6 p-4 th-card border border-slate-800 w-full"):
                        ui.label("Talent Sourcing & Recruitment Funnel").classes(
                            "text-base font-semibold text-slate-200 mb-2"
                        )
                    ...
                    with ui.card().classes("col-6 p-4 th-card border border-slate-800 w-full"):
                        ui.label("Sourcing & Outreach Activity Trends").classes(
                            "text-base font-semibold text-slate-200 mb-2"
                        )
                ...
                with ui.row().classes("w-full gap-6 no-wrap-md"):
                    ...
                    with ui.card().classes("col-6 p-4 th-card border border-slate-800 w-full"):
                        ui.label("Recruitment Velocity (Time-to-Fill Days per Hunt)").classes(
                            "text-base font-semibold text-slate-200 mb-2"
                        )

                        hunts_vel = velocity_data.get("hunts_velocity", [])
                        h_titles = [
                            (h.get("title") or "Untitled")[:22] + "..."
                            if len((h.get("title") or "Untitled")) > 22
                            else (h.get("title") or "Untitled")
                            for h in hunts_vel
                        ]
                    ...
                    with ui.card().classes("col-6 p-4 th-card border border-slate-800 w-full"):
                        ui.label("Candidate Quality & Match Score Distribution").classes(
                            "text-base font-semibold text-slate-200 mb-2"
                        )
                ...
                with ui.row().classes("w-full gap-6"):
                    with ui.card().classes("w-full p-4 th-card border border-slate-800"):
                        with ui.row().classes("justify-between items-center w-full mb-2"):
                            ui.label("AI Engine Execution & Cost Optimization Tracker").classes(
                                "text-base font-semibold text-slate-200"
                            )
                            ui.chip("Local Edge LLM Enabled", icon="memory", color="teal").classes(
                                "text-xs"
                            )

                        with ui.row().classes(
                            "w-full items-center gap-8 py-2 px-4 bg-slate-900/60 rounded-lg border border-slate-800 mb-4"
                        ):
                            with ui.column().classes("gap-0"):
                                ui.label("Total AI Operations").classes("text-xs text-slate-400")
                                ui.label(str(ai_cost_data.get("total_operations", 0))).classes(
                                    "text-xl font-bold text-slate-100"
                                )
                            with ui.column().classes("gap-0"):
                                ui.label("Recorded Local GGUF").classes("text-xs text-slate-400")
                                ui.label(f"{ai_cost_data.get('local_operations', 0)} ops").classes(
                                    "text-xl font-bold text-teal-400"
                                )
                            with ui.column().classes("gap-0"):
                                ui.label("Recorded Cloud API").classes("text-xs text-slate-400")
                                ui.label(f"{ai_cost_data.get('cloud_operations', 0)} ops").classes(
                                    "text-xl font-bold text-amber-400"
                                )
                            with ui.column().classes("gap-0"):
                                ui.label("Provider Unattributed").classes("text-xs text-slate-400")
                                ui.label(
                                    f"{ai_cost_data.get('unattributed_operations', 0)} ops"
                                ).classes("text-xl font-bold text-slate-300")
                            with ui.column().classes("gap-0"):
                                ui.label("Actual Cloud API Cost").classes("text-xs text-slate-400")
                                ui.label(f"${ai_cost_data.get('actual_cloud_cost', 0.0)}").classes(
                                    "text-xl font-bold text-slate-200"
                                )
                            with ui.column().classes("gap-0"):
                                ui.label("Net Savings vs Full Cloud").classes(
                                    "text-xs text-slate-400"
                                )
                                ui.label(f"${ai_cost_data.get('total_cost_saved', 0.0)}").classes(
        ...
        def render_tables():
            ...
            with tables_container:
                ...
                with ui.card().classes("w-full p-4 th-card border border-slate-800"):
                    ui.label("Campaign Performance & Velocity Breakdown").classes(
                        "text-base font-semibold text-slate-200 mb-3"
                    )
                    ...
                    ui.table(columns=columns, rows=rows, row_key="hunt_id").classes(
                        "w-full bg-transparent text-slate-200"
                    ).props("flat")
        ...
        def handle_export(report_format: str):
            hunt_id = None if state["selected_hunt_id"] == 0 else state["selected_hunt_id"]
            result = dispatch_action(
                ...
                actor_type="ui",
                session_id="analytics-ui",
            )
            if not result.success:
                ui.notify(result.error or "Report generation failed.", type="negative")
                return
            artifact = result.data["artifact"]
            ui.download(artifact["download_url"], filename=artifact["file_name"])
            ui.notify(
                f"{report_format.upper()} report created from canonical analytics.",
                type="positive",
            )
